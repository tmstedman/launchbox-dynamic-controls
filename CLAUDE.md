...
4. Entries are `<Input name="BUTTON1">label</Input>`; the `name` attribute is the platform button and the element text is the label, trimmed on read. A space-separated `name` is a button combination: it resolves to the inputs *every* named button drives (the intersection) and outranks those buttons' individual labels there. An empty intersection renders nothing. Two combinations can legitimately overlap on one generic — e.g. three buttons that all fire together on one shared trigger will also, in every pairwise combination among them, reach that same generic alongside each pair's own distinct trigger — so the combination naming *more* buttons wins there; equally-specific combinations contending for one generic log at `Error` and keep the last, same as unresolvable individual-button contention below
