...
**Whole-input derivation** (`WholeInputDeriver`, applied in `InputMappingService` after the naturals are spliced): a platform button naming a whole control (`JOYSTICK`, `Dpad-Any`) can't be moved by an emulator config, which binds directions individually. So after a transform, each whole-input button is extended to the controls its direction siblings now drive. The whole-and-parts pairing is discovered from `NaturalButtonToInput`, never parsed from names. Three rules: a whole is claimed only when **all four** of its directions are covered; derived inputs are **appended**, so a directly-mapped button outranks one that merely reached the control (and a derived binding never displaces an existing `InputToButton` entry); and only emulator-derived layers are derived over — a per-game `InputMappings` file can name a whole control itself, so its silence is taken as intent. **RetroArch is not yet covered** — it's a source, not a transform, so its remaps are already inside the natural snapshot and the pairing is gone by the time the deriver runs (issue #6).
