...
|---|---|---|
| `DefaultTemplate` | `Xbox Series X` | Controller artwork to draw - a folder name under `Templates\`. Leave empty to disable the overlay. |
| `Debug` | `false` | `true` writes a verbose `Logs\debug.log` on every launch. |
| `EnableMame` | `false` | `true` reads your MAME `.cfg` files to pick up game-specific JOYCODE button assignments. |
...
Generic input names (`ButtonA`, `ButtonB`, `ButtonX`, `ButtonY`, `ButtonLeftShoulder`, `ButtonDpad`, `AxisLeftStick`, …) are the named slots in the controller template - each corresponds to a button image and position. The mapping connects original hardware buttons to those slots. So the Genesis `B` button is displayed using the `ButtonA` slot (the A position on an Xbox controller). This file ships for ~50 platforms.
...
The plugin automatically reads a `controls.xml` database to display what each button does in each game. `controls.xml` is not distributed with the plugin: download the BYOAC MAME controls database and place it at `Data\Dynamic Controls\controls.xml`. If the file is absent, no automatic labels are shown for MAME games.
