...
Controls whether a `<Render>` or `<Overlay>` is at full opacity, or rendered at `minOpacity` with a `inactiveBlurRadius` blur instead. Allowed values:

- `label` — full opacity when this input has a label, dimmed otherwise
- `mapping` — full opacity when a platform button drives this input, dimmed otherwise
...
The unit of the layout. An Input has a `name` matching a generic input identifier (`ButtonA`, `AxisLeftStickUp`, etc.) and contains the renders, labels, and overlays that visualise it.
...
A vertically-stacked cluster. Children are positioned at `(x, y + slotIndex × gap)`. Unlike Group, the Stack is always included regardless of whether children have visible renders — children handle their own visibility.
...
| `y` | coordinate | no | Same. Default `+0` |
| `gap` | double | no | Vertical spacing between slot indices. Default 0 |
| `collapse` | bool (`true`/anything-else) | no | When `true`, slots whose contents are all dim vacate the slot and following slots shift up by `gap` |

**Children** can be `<Input>`, `<Group>`, `<Stack>`, `<OneOf>`, `<Overlay>` in any order.

**Slot counting** in a Stack:

| Child kind | Slot behaviour |
|---|---|
| `<Input>` | Consumes one slot |
| `<Group>` (plain) | Transparent — its children each consume a slot |
| `<Stack>` (nested) | Consumes one slot as a block; the inner Stack has its own slot count internally |
| `<OneOf>` | Consumes one slot; all alternatives share the slot's origin |
| `<Overlay>` | No slot impact — rendered at its own coordinates |

**Collapse** is opt-in per Stack. When set, the LayoutFilter checks at render time which slot members have all-zero-opacity images; vacated slots shift subsequent members up by `gap`. Without collapse, slots stay fixed.
